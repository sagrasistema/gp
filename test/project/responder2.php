...
<div class="meta-summary" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 2rem; padding: 1.25rem; border-radius: 12px; background: #ffffff; border: 1px solid var(--border-color, #cbd5e1);">
...
<!-- Titular y Botonera Superior de Acciones -->
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
    <h1 style="font-size: 1.5rem; font-weight: 700; color: #0f172a; margin: 0; display: flex; align-items: center; gap: 0.5rem;">
        <i class="ri-dashboard-line" style="color: var(--accent, #00bcd4);"></i>Etapa 2: Estrategia
    </h1>
    ...
    <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid #e2e8f0; display: grid; grid-template-columns: repeat(12, 1fr); gap: 1rem;">
        <div style="grid-column: span 6; background: #ffffff; padding: 0.75rem; border-radius: 6px; border: 1px solid #f1f5f9;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; font-size: 0.85rem; font-weight: 600; color: #475569;">
                ...
                <span style="color: #0f172a; font-weight: 700;"><?= $porcentajeProgreso ?? 0 ?>%</span>
            </div>
            <div style="width: 100%; background-color: #e2e8f0; height: 10px; border-radius: 9999px; overflow: hidden;">
                <div style="width: <?= $porcentajeProgreso ?? 0 ?>%; background-color: #10b981; height: 100%; border-radius: 9999px; transition: width 0.4s ease;"></div>
            </div>
        </div>
        ...
        <div style="grid-column: span 6; background: #ffffff; padding: 0.75rem; border-radius: 6px; border: 1px solid #f1f5f9;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; font-size: 0.85rem; font-weight: 600; color: #475569;">
                ...
                <span style="color: #0f172a; font-weight: 700;"><?= $porcentajeProgresoo ?? 0 ?>%</span>
            </div>
            <div style="width: 100%; background-color: #e2e8f0; height: 10px; border-radius: 9999px; overflow: hidden;">
                <div style="width: <?= $porcentajeProgresoo ?? 0 ?>%; background-color: #3b82f6; height: 100%; border-radius: 9999px; transition: width 0.4s ease;"></div>
            </div>
        </div>
    </div>
</div>
...
<div class="accordion-container">
    <?php
    $stmtCat = $pdo->prepare("SELECT * FROM audit_categorias WHERE etapa_id = 2 ORDER BY orden ASC");
    $stmtCat->execute();
    ...
    ?>
        <div class="accordion-item" style="margin-bottom: 0.75rem; border: 1px solid var(--border-color, #cbd5e1); border-radius: 8px; overflow: hidden;">
            <div class="accordion-header" onclick="toggleAccordion(this)" style="background: #f1f5f9; padding: 1rem; font-weight: 700; cursor: pointer; display: flex; justify-content: space-between; align-items: center;">
                ...
                <i class="ri-arrow-down-s-line" style="font-size: 1.2rem; transition: transform 0.2s ease;"></i>
            </div>
                    ...
                    <div class="prueba-row-container">
                        ...
                        <div class="prueba-actions">
                            <div style="display: flex; align-items: center; gap: 0.35rem;">
                                ...
                                ?>
                                <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasCI ? '#ca8a04' : '#cbd5e1' ?>; background: <?= $hasCI ? '#fef9c3' : '#f8fafc' ?>; color: <?= $hasCI ? '#ca8a04' : '#94a3b8' ?>;" title="Debilidades de Control Interno">
                                    CI
                                </span>
                                <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasCG ? '#ea580c' : '#cbd5e1' ?>; background: <?= $hasCG ? '#ffedd5' : '#f8fafc' ?>; color: <?= $hasCG ? '#ea580c' : '#94a3b8' ?>;" title="Carta de Gerencia">
                                    CG
                                </span>
                                ...
                                <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasAA ? '#2563eb' : '#cbd5e1' ?>; background: <?= $hasAA ? '#dbeafe' : '#f8fafc' ?>; color: <?= $hasAA ? '#2563eb' : '#94a3b8' ?>;" title="Asuntos de Auditoría">
                                    AA
                                </span>
                            </div>
