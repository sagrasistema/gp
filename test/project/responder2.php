...
// 2. Cargar dependencias y lógica de backend
include '../main/config.php';
include 'conect-proyecto2.php';

$pageTitle = "Panel de Control de Auditoría - Fase 2: Estrategia";
include '../main/h.php';
?>
<link rel="stylesheet" href="../main/layout.css">
<style>
...
<div class="view-container">
    ...
    <!-- Barra de Navegación Rápida por Etapas del Proyecto -->
    <div class="project-stages-bar">
        <a href="responder.php?proyectoId=<?= (int)$proyectoId ?>" class="stage-btn">
            <i class="ri-calendar-check-line"></i>1. Planificación
        </a>
        <a href="responder2.php?proyectoId=<?= (int)$proyectoId ?>" class="stage-btn active">
            <i class="ri-compass-3-line"></i>2. Estrategia
        </a>
        <a href="responder3.php?proyectoId=<?= (int)$proyectoId ?>" class="stage-btn">
            <i class="ri-play-circle-line"></i>3. Ejecución
        </a>
        <a href="responder4.php?proyectoId=<?= (int)$proyectoId ?>" class="stage-btn">
            <i class="ri-flag-line"></i>4. Conclusión
        </a>
    </div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
        ...
        <div style="display: flex; align-items: center; gap: 0.35rem; margin-left: auto;">
            ...
            <a href="#" class="btn btn-primary" data-tooltip="Reporte general" onclick="return false;">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
            <?php if (!empty($pruebasList)): ?>
                ...
                foreach ($pruebasList as $prueba): 
                    ...
                    if (in_array($estadoPrueba, ['completado', 'cerrado'], true)) {
                    ...
                    if ((int)($ejecucion['texto_inadecuado'] ?? 0) === 1 || (int)($ejecucion['texto_inadecuado2'] ?? 0) === 1) {
                        $bgColor = '#ef4444';
                    }
                ...
                ?>
                    <a href="actividades.php?proyectoId=<?= (int)$proyectoId ?>&pruebaId=<?= $safeId ?>" 
        ...
        <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid #e2e8f0; display: grid; grid-template-columns: repeat(12, 1fr); gap: 1rem;">
            <div style="grid-column: span 6; background: #ffffff; padding: 0.75rem; border-radius: 6px; border: 1px solid #f1f5f9;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; font-size: 0.85rem; font-weight: 600; color: #475569;">
                    ...
                    <span style="color: #0f172a; font-weight: 700;"><?= (int)($porcentajeProgreso ?? 0) ?>%</span>
                </div>
                <div style="width: 100%; background-color: #e2e8f0; height: 10px; border-radius: 9999px; overflow: hidden;">
                    <div style="width: <?= (int)($porcentajeProgreso ?? 0) ?>%; background-color: #10b981; height: 100%; border-radius: 9999px; transition: width 0.4s ease;"></div>
                </div>
            </div>

            <div style="grid-column: span 6; background: #ffffff; padding: 0.75rem; border-radius: 6px; border: 1px solid #f1f5f9;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; font-size: 0.85rem; font-weight: 600; color: #475569;">
                    <span>Progreso de tareas Revisadas</span>
                    <span style="color: #0f172a; font-weight: 700;"><?= (int)($porcentajeProgresoo ?? 0) ?>%</span>
                </div>
                <div style="width: 100%; background-color: #e2e8f0; height: 10px; border-radius: 9999px; overflow: hidden;">
                    <div style="width: <?= (int)($porcentajeProgresoo ?? 0) ?>%; background-color: #3b82f6; height: 100%; border-radius: 9999px; transition: width 0.4s ease;"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="accordion-container">
        <?php
        $stmtCats = $pdo->prepare("SELECT * FROM audit_categorias WHERE etapa_id = 2 ORDER BY orden ASC");
        $stmtCats->execute();
        $categories = $stmtCats->fetchAll(PDO::FETCH_OBJ);
                        ...
                        <div class="prueba-row-container">
                            ...
                            <div class="prueba-actions">
                                <div style="display: flex; align-items: center; gap: 0.35rem;">
                                    ...
                                    ?>
                                    <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasCI ? '#ca8a04' : '#cbd5e1' ?>; background: <?= $hasCI ? '#fef9c3' : '#f8fafc' ?>; color: <?= $hasCI ? '#ca8a04' : '#94a3b8' ?>;" title="Debilidades de Control Interno (Amarillo)">CI</span>
                                    <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasCG ? '#ea580c' : '#cbd5e1' ?>; background: <?= $hasCG ? '#ffedd5' : '#f8fafc' ?>; color: <?= $hasCG ? '#ea580c' : '#94a3b8' ?>;" title="Carta de Gerencia (Naranja)">CG</span>
                                    <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasSC ? '#dc2626' : '#cbd5e1' ?>; background: <?= $hasSC ? '#fee2e2' : '#f8fafc' ?>; color: <?= $hasSC ? '#dc2626' : '#94a3b8' ?>;" title="Situaciones Críticas (Rojo)">SC</span>
                                    <span style="font-size: 0.75rem; font-weight: 700; padding: 0.2rem 0.45rem; border-radius: 4px; border: 1px solid <?= $hasAA ? '#2563eb' : '#cbd5e1' ?>; background: <?= $hasAA ? '#dbeafe' : '#f8fafc' ?>; color: <?= $hasAA ? '#2563eb' : '#94a3b8' ?>;" title="Asuntos de Auditoría (Azul)">AA</span>
                                </div>
                                ...
                                <a href="actividades.php?proyectoId=<?= (int)$proyectoId ?>&pruebaId=<?= (int)$pr->id ?>" class="btn btn-primary" style="padding: 0.4rem 0.75rem; font-size: 0.85rem;" data-tooltip="Llenar Cuestionario y Gestionar Estatus">
